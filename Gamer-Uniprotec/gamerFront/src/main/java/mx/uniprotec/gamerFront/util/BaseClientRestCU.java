...
//			    	log.info(resultVO.toString());
				...
				rs.setMensaje(e.getMessage().concat(": ")
...
//			log.info(token);
		...
		RestTemplate restTemplate = new RestTemplate();
		try {
			ResponseEntity<JSONObject> response = restTemplate.exchange(URL_CRUD + urlCrud, HttpMethod.POST, entity,
					JSONObject.class);
//			    resultVO = asignaResponseObject(response);
			...
			ResultVO rs = new ResultVO();
			rs.setJsonResponse(jsonResponse);
			rs.setMensaje("Error:" + e.getMessage().concat(": ")
					.concat(((NestedRuntimeException) e).getMostSpecificCause().getMessage()));
			rs.setCodigo(Long.valueOf(HttpStatus.INTERNAL_SERVER_ERROR.value()));
			e.printStackTrace();
	...
	private ResultVO getTemplateObjetoPut(String token, String urlCrud, Object objeto, Long idObjeto) {
		...
		RestTemplate restTemplate = new RestTemplate();
		try {
			ResponseEntity<JSONObject> response = restTemplate.exchange(urlPut, HttpMethod.PUT, entity,
					JSONObject.class);
			return asignaResponseObject(response);
		} catch (Exception e) {
			JSONObject jsonResponse = new JSONObject();
	...
	private ResultVO getTemplateObjetoGetAll(String token, String urlCrud) {
		...
		RestTemplate restTemplate = new RestTemplate();
		try {
			ResponseEntity<JSONObject> response = restTemplate.exchange(URL_CRUD + urlCrud, HttpMethod.GET, entity,
					JSONObject.class);
//			    log.info(resultVO.toString());
//			    resultVO = asignaResponseObject(response);
	...
	private ResultVO getTemplateObjetoGetId(String token, String urlCrud, Object object, String idObject) {
		...
		RestTemplate restTemplate = new RestTemplate();
		try {
			ResponseEntity<JSONObject> response = restTemplate.exchange(urlGetId, HttpMethod.GET, entity,
					JSONObject.class);
//			    log.info(resultVO.toString());
//			    resultVO = asignaResponseObject(response);
